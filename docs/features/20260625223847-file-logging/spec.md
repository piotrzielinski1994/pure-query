...
- AC-001: A pure helper produces the per-launch log file stem `purequery-<YYYYMMDDHHMMSS>` (14 zero-padded digits) from decomposed local date/time components.
- AC-002: On app launch, file logging is registered best-effort - one file `purequery-<timestamp>.log` per launch in the OS app-log dir (macOS `~/Library/Logs/com.pzielinski.purequery/`), plus stdout, at `Info` level, `KeepAll` rotation, 50 MB max size.
...
- TC-001 (happy path): `launch_log_name(2026, 6, 25, 22, 38, 47)` -> `"purequery-20260625223847"`. Maps to: AC-001
- TC-002 (edge - padding): `launch_log_name(2026, 1, 2, 3, 4, 5)` -> `"purequery-20260102030405"`. Maps to: AC-001
- TC-003 (shape): stem after `purequery-` is exactly 14 ASCII digits. Maps to: AC-001
...
- No new frontend npm deps (`@tauri-apps/api` already present). No `log:default` capability needed: `log:default` gates the official `@tauri-apps/plugin-log` JS API, which purequery never imports - the frontend routes through our own custom `log_message` command, permitted by `core:default`. (requi keeps `log:default` because of unrelated history; it is not required here.)

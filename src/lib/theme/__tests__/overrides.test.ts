...
//    it (AC-006 sparse store = per-token reset).
...
describe("applyDefaults", () => {
  // AC-004, AC-005 - behavior: an overridden token wins; the others fall back.
  ...
  // AC-004 - behavior: the full effective set carries EVERY default token (so all
  ...
  // AC-007 - behavior: overrides for the two modes are independent.
  ...
  // AC-009 (editor half) - behavior: editor overrides layer the same way.
...
describe("diffOverrides", () => {
  // AC-006 - behavior: only entries differing from the default are kept.
  ...
  // AC-006 - behavior: a token edited BACK to its default drops out (the reset).
  ...
  // AC-006 - behavior: whitespace-insensitive compare treats spacing variants as
  ...
  // AC-006 - behavior: a genuinely different value (despite shared prefix) stays.
  ...
  // AC-006 (editor half) - behavior: editor-token diffing works the same way.
...
describe("diffOverrides / applyDefaults round-trip", () => {
  // AC-006, AC-007 - behavior: diff(apply(x, d), d) deep-equals x for a
  ...
  // AC-006 - behavior: an empty sparse set survives the round-trip as empty (no

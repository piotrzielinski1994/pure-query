...
// jsdom has no matchMedia - controllable stub.
...
describe("ThemeProvider colors", () => {
  ...
  // AC-004 - behavior: effectiveColors merges overrides over the built-in defaults.
  ...
  // AC-004 - behavior: an un-overridden effective token tracks its built-in default.
  ...
  // AC-005, TC-005 - side-effect-contract: a light primary override sets --primary
  // on the <html> element when the effective mode is light.
  ...
  // AC-007, TC-007 / spec §6 - side-effect-contract: a light-only override leaves
  // the var CLEARED while the effective mode is dark (dark has no override).
  ...
  // AC-006 - side-effect-contract: setColors persists through saveThemeColors
  // (asserted via the save spy's payload).
  ...
  // AC-005 - side-effect-contract: applying an override via setColors inlines the
  // var on <html> (the colors flow back through context to the layout effect).
  it("should inline a setColors override var on the html element", async () => {
    const user = userEvent.setup();

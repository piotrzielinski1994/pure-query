...
// An empty folder has no child rows to drop near, so during a drag it renders a
// dedicated child-area drop zone with this id. Hovering it always means "drop
// inside the folder", giving a large, reliable target.
...
// - Database/table row: top half = before, bottom half = after (reorder only;
//   never inside - a database is not a container).
// - Collapsed folder row: top/bottom quarters reorder, middle 50% = inside.
// - Expanded folder row: only a thin top strip = before (reorder above it); the
//   rest = inside. "After an open folder" visually IS its children area, so the
//   whole non-top row reliably reparents - no narrow middle band.

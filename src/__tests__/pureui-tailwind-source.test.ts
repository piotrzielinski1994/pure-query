...
describe("app Tailwind build scans @pziel/pureui", () => {
  ...
  it("emits the pureui context-menu / scroll-area utilities when scanned", () => {
    const scanner = new Scanner({
      sources: [{ base: PUREUI_DIST, pattern: "**/*", negated: false }],
    });
    const candidates = scanner.scan();

    // These utilities live ONLY inside pureui's hoisted context-menu
    // (min-w-[10rem]) and scroll-area (bg-foreground/20) components; if the app
    // doesn't scan pureui they never get generated.
    expect(candidates).toContain("min-w-[10rem]");
    expect(candidates).toContain("bg-foreground/20");
  });
});
